...
補足:
- `docker compose up -d` で `frontend` サービスを起動すると、`docker-compose.yml` の `command` により Vite はすでに `0.0.0.0:5173` で起動します。
- その状態で `docker compose exec frontend npm run dev` を実行すると、2つ目の Vite が起動し、`5173` 使用中のため `5174` にフォールバックします。
- 通常は `npm run dev` を重ねて実行せず、`docker compose logs -f frontend` で状態確認してください。
